...
import xmlrpclib, pybru, sys, getopt, time, traceback
...
def crawl(port = 10000, logger = null_logger, debug = False):
  ...
  while True:
    try:
      ...
      if retry_count > 0 or no_response_count > 0:
        res = rpc.proxy(node, 3, 1, "sys:link.GetNeighbors")
        if len(res) == 0:
          raise Exception("Unable to communicate, try again...")

      res = rpc.proxy(node, 3, 1, "Information.Info")
      if len(res) == 0:
        res = rpc.proxy(node, 3, 1, "sys:link.GetNeighbors")
        if len(res) == 0:
          raise Exception("Unable to communicate, try again...")
      ...
      if no_response_count == no_response_max:
        ...
        if retry_count ==  retry_max:
          if trying_right2:
            logger("Unable to crawl the system.")
            break
          else:
            trying_right2 = True
            retry_count = 0
